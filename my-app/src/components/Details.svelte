<script lang="ts">
	...
	import type { Ufos } from './ufo_data';
	import { shared, type SelectionType } from './shared';
	import Modal from './Modal.svelte';
	let showModal = false;
	let clazz = '';
	export { clazz as class };
	let shared_state: SharedState | null;
	let selected_type: SelectionType;
	...
	shared.subscribe((v) => {
		...
		selected_ufos = null; // v.selected_type == 'ufo' ? (v.selected as Ufos) : null;
		counter = 0;
	});
</script>
...
<div class="{clazz} details card">
	...
	{:else if selected_type == 'state'}
		...
		<b>Religion</b>
		<p class="description">TODO</p>
	{:else if selected_type == 'ufo' && selected_ufos !== null}
