<div class="scatterplot">
	...
	<div id="controls">
		...
		<p>Select attribute for x axis: <!--changed for x axis-->
        	<span class="xvar">
        	<select class="xvar" name="xvar"></select>
      	</span></p>
		<p>
	...
	<script>
		...
		let state_indices = //changed for full rel
		['AK',
		'AL',
		'AR',
		'AZ',
		...
		// load data
		let url_other = "https://raw.githubusercontent.com/vfrawa/vfrawa.github.io/main/scatterplot_data_full_relative_drugsincl.csv"
		...
		//let url_sight = "https://raw.githubusercontent.com/useless-fisualization-operation/ufo-web/main/dataset/num_sightings_per_state_per_year.csv"
		let url_sight = "https://raw.githubusercontent.com/vfrawa/vfrawa.github.io/main/relative_num_sightings_per_state_per_year_timespan_rows_full.csv"
		let url_sight_abs = "https://raw.githubusercontent.com/vfrawa/vfrawa.github.io/main/num_sightings_per_state_per_year_timespan_rows_full.csv" //changed for x axis
		d3.csv(url_other, cleanup_data)
		.then(d_other => {
			d3.csv(url_sight, cleanup_data)
			.then(d_sight => {
				//setup(d_other, d_sight)
				//update(d_other, d_sight)
				d3.csv(url_sight_abs, cleanup_data) //changed for x axis
				.then(d_sight_abs => {
		...
		let xScale, yScale
		let xAxis, yAxis
		let min_year = 1910, max_year = 2023

		function get_state_year_idx(state, start_year, end_year) {//changed for full rel
		state_idx = state_indices.indexOf(state);
		start_year_idx = start_year-min_year;
		end_year_idx = end_year-min_year;
		return state_idx*6555+(114.5-start_year_idx/2.0)*start_year_idx + end_year_idx -start_year_idx;
		}
		...
		function setup(d_other, d_sight, d_sight_abs) { //changed for x axis
		...
		let variables = Object.keys(d_other[0]).filter(d => d != 'State' && d!="Rel_Sample_size" & d!="Num_Sightings" & d!="Rel_Num_Sightings") //changed for full rel
		...
		d3.select("#button_start_year").property('max', d3.select("#button_end_year").property('value'))
		d3.select("#button_end_year").property('min', d3.select("#button_start_year").property('value'))
		...
		d_other.map(d => {
			...
			if (xvar == 'Num_Sightings') { //changed for x axis
		...
		scatterOuter
			...
			.attr(
			'transform',
			`rotate(-90 ${margins.left / 2} ${margins.top + innerHeight / 2})`
			)
			.text(longVars[yvar])
		...
		scatterInner
			...
			enter =>
				...
				.append('circle')
					...
					.attr('cx', d => xScale(d["Rel_Num_Sightings"]))//changed for x axis
					.attr('cy', d => yScale(d[yvar]))
					.style('fill', 'black')
					.attr('r', d => 4)
					.style('opacity', 0.6)
					.append("title") // TITLE APPENDED HERE
					.text(function(d) { return longStates[d["State"]]; }),
			update =>
				update
				.transition()
				.duration(transitionTime)
